...
def createDataFrame(rdd):
    try:
        ...
        try:
            model=joblib.load("SGD_"+str(trainy.shape[0])+'.pkl')
            model.partial_fit(trainx, trainy,classes=np.unique(trainy))
            joblib.dump(model,"SGD_"+str(trainy.shape[0])+'.pkl')
        ...
        try:
            ...
            if(fsc>maxfsc):
                maxfsc=fsc
                joblib.dump(model,"SGD_bestfsc_"+str(trainy.shape[0])+'.pkl')
            facp=open('./SGD_stats_'+str(trainy.shape[0])+'.txt','w+')
            facp.write("current f1:"+str(fsc)+"\nbest f1:"+str(maxfsc)+"\nbatchsize:"+str(trainy.shape[0]))
            facp.close()
            print("f1 score",fsc)
            cm = confusion_matrix(test_y, y_pred)
            print(cm)
...
joblib.dump(classifier,"SGD_800.pkl")
maxfsc=0
